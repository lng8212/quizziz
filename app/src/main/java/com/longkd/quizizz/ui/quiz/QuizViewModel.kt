...
import com.longkd.quizizz.model.Leaderboard
import com.longkd.quizizz.model.QuestionsModel
...
class QuizViewModel @Inject constructor(
    ...
    val user: LiveData<User> = firebaseUser.map { user ->
        if (user != null) {
            User(
                user.uid,
                user.displayName,
    ...
    private suspend fun updateLeaderboards() {
        withContext(Dispatchers.IO) {
            try {
                ...
                val oldResult = value?.toObject<com.longkd.quizizz.model.Result>()
